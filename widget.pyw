...
def lighten_color(hex_color, amount=25):
    try:
        # support shorthand 3‑digit hex like "#abc" by expanding it
...
ANIMATION_SPEED_MS = 12      # increased from 8 for smoother 60fps
ANIMATION_SMOOTHNESS = 0.4   # increased from 0.25 for quicker animations
...
class WindowsBlur:
    def __init__(self, window):
        try:
            ...
            self.accent_policy = self.AccentPolicy()
            self.win_comp_attr_data = self.WindowCompositionAttributeData()
            self.SetWindowCompositionAttribute = ctypes.windll.user32.SetWindowCompositionAttribute
            self.SetWindowCompositionAttribute.argtypes = [ctypes.c_int, ctypes.POINTER(self.WindowCompositionAttributeData)]
            self.SetWindowCompositionAttribute.restype = ctypes.c_int
        except Exception: pass
    ...
    class WindowCompositionAttributeData(ctypes.Structure):
        _fields_ = [("Attribute", ctypes.c_int), ("Data", ctypes.POINTER("AccentPolicy")), ("SizeOfData", ctypes.c_int)]

    def apply_blur(self):
        try:
            self.accent_policy.AccentState = 0
            self.accent_policy.GradientColor = 0xCC1A1A1A
            self.win_comp_attr_data.Attribute = 19
            self.win_comp_attr_data.Data = ctypes.pointer(self.accent_policy)
            self.win_comp_attr_data.SizeOfData = ctypes.sizeof(self.accent_policy)
            self.SetWindowCompositionAttribute(self.hwnd, ctypes.pointer(self.win_comp_attr_data))
        except Exception: pass
...
class TaskWidget(ctk.CTk):
    def __init__(self):
        ...
        # cache for parsed dates to avoid repeated strptime calls
        self._date_cache = {}
        # helpers for animation / refresh state
        self.is_animating = False
        self.anim_job = None                # holds after job id for height animation
        self.is_refreshing = False          # avoid concurrent refreshes
        ...
        self.expanded = False
        # self.is_animating already initialised above
        ...
        # start at top-right corner with small margin
        margin = 12
        ...
        # --- HEADER ---
        self.header_frame = ctk.CTkFrame(self, corner_radius=10, fg_color=("#000000", "#2b3542"), bg_color=self.transparent_color_key)
        ...
        self.loading_label = ctk.CTkLabel(self.title_container, text="Betöltés...", font=FONT_TEXT_BOLD, text_color="#AAAAAA")
        ...
        btn_args = {"width": 36, "height": 30, "font": FONT_ICON, "fg_color": "transparent", "hover_color": "#444"}
        self.btn_refresh = ctk.CTkButton(self.button_container, text="\uE72C", command=self.start_refresh, **btn_args)
        self.btn_refresh.pack(side="left", padx=2)
        self.btn_toggle = ctk.CTkButton(self.button_container, text="\uE70D", command=self.toggle_size_animated, **btn_args)
        self.btn_toggle.pack(side="left", padx=2)
        self.btn_close = ctk.CTkButton(self.button_container, text="\uE8BB", command=self.close_app, text_color="#AAAAAA", **btn_args)
        self.btn_close.configure(hover_color="#CC5252")
        self.btn_close.pack(side="left", padx=2)

        # --- SCROLL FRAME ---
        # SMEARING FIX: bg_color szolid → a canvas háttere solid, nem átlátszó
        # A corner_radius vizuálisan megmarad, de a canvas nem fog elmosódni scrollnál.
        self.scroll_frame = ctk.CTkScrollableFrame(self, corner_radius=10, fg_color=("#1a2428", "#2b3542"), bg_color=("#1a2428", "#2b3542"))
        
        # --- ADD TASK UI ---
        self.expandable_add_task_container = ctk.CTkFrame(self.scroll_frame, fg_color="transparent")
        self.expandable_add_task_container.pack(fill="x", padx=5, pady=5)
        
        # the add-section starts collapsed; a single button toggles visibility
        ...
        # do not pack the frame yet; toggle_add_section will handle showing it
        #self.add_task_frame.pack(fill="x", padx=5, pady=(5, 10))
        
        self.bucket_var = ctk.StringVar(value=list(PLANNER_CONFIG.keys())[0])
        self.bucket_menu = ctk.CTkOptionMenu(self.add_task_frame, values=list(PLANNER_CONFIG.keys()), variable=self.bucket_var, font=FONT_TEXT_SMALL, height=32,corner_radius=6,fg_color="#1f1f1f",button_color="#1f1f1f",button_hover_color="#333333",dropdown_fg_color="#1f1f1f",dropdown_hover_color="#A16565",dropdown_text_color="white",dynamic_resizing=False,anchor="w")
        self.bucket_menu.configure(fg_color="#3B535A", button_color="#1f1f1f", dropdown_fg_color="#2E2828",dropdown_hover_color="#3B535A")
        self.bucket_menu.pack(fill="x", padx=10, pady=(10, 5))
    
        # Név beviteli mező
        self.task_entry = ctk.CTkEntry(self.add_task_frame, placeholder_text="Új feladat neve...", font=FONT_TEXT_NORMAL, height=35)
        self.task_entry.pack(fill="x", padx=10, pady=(0, 5))
        # clear error indication when user types any key
        ...
        self.date_entry = ctk.CTkEntry(entry_row, placeholder_text="ÉÉÉÉ-HH-NN", font=FONT_TEXT_SMALL, height=35, width=120)
        self.date_entry.insert(0, today_str)
        self.date_entry.pack(side="left", padx=(0, 5))
        
        self.btn_add = ctk.CTkButton(entry_row, text="Hozzáadás +", height=35, font=("Segoe UI", 13, "bold"), command=self.add_new_task, anchor="center", compound="right")
        self.btn_add.pack(side="right", expand=True, fill="x")

        # Konténer a feladatoknak
    ...
    def _parse_date(self, date_str):
        """Parse and cache date strings to avoid repeated strptime calls."""
    ...
    def is_valid_date(self, date_string):
        """Check date format and component ranges.
        Returns a tuple `(valid: bool, error_message: str|None)` where the message
        is suitable for display when valid is False.
        """
        if not date_string:
            return True, None
        ...
        # day range depends on month/year
        try:
            import calendar
    ...
    def toggle_add_section(self):
        ...
        if self.is_add_section_open:
            ...
            # if the main window is expanded we leave size alone (always at max_height)
            if self.expanded:
                # still need to re-enable button immediately since no anim
                self.btn_toggle_add.configure(state="normal")
            else:
                self.btn_toggle_add.configure(state="normal")

    def add_new_task(self, event=None):
        # ignore repeated calls while request in progress
        ...
        if not title:
            ...
            # restore normal border in case it previously turned red
            self.task_entry.configure(border_color=["#979797", "#565b5e"])
        ...
        # Egyszerű validálás: ha üres vagy nem jó a formátum, a backend hibaágra futhat,
        # de itt alapértelmezetten a mait küldjük ha üres
        ...
        def worker():
            if backend.create_task(title, bucket_id, plan_id, due_date):
                ...
                # close the add section after successful creation
    ...
    def show_status_message(self, message, color="#AAAAAA"):
        for w in self.title_container.winfo_children(): w.destroy()
        if message:
            ctk.CTkLabel(self.title_container, text=message, font=FONT_TEXT_BOLD, text_color=color).pack(side="left")
    ...
    def _clear_name_error(self):
        # remove red outline from entry and clear leading status if "Hiányzó név"
        try:
            self.task_entry.configure(border_color=["#979797", "#565b5e"])
        except Exception:
            pass
        # if message field contains only error, clear it as well
    ...
    def delete_task(self, task_id, title=""):
        # show visual feedback then call backend
        self.show_status_message("Törlés...", "#FF5555")
        # remove from UI immediately for a snappier experience
        ...
        def worker():
            ...
            if not ok:
                self.after(0, lambda: self.show_status_message("Törlés sikertelen", "#FF7777"))
            # whether success or not, refresh data to stay in sync
            self.after(0, lambda: self.start_refresh(skip_intro=True))
        threading.Thread(target=worker, daemon=True).start()
    ...
    def toggle_size_animated(self):
        # simple expand/collapse with fixed heights to avoid expensive layout recalcs
        if self.is_animating:
            # queue another toggle request instead of ignoring? just return for simplicity
            return
        ...
        if self.expanded:
            ...
            self.expanded = False
            self.animate_height(self.winfo_height(), self.min_height)
        else:
            self.btn_toggle.configure(text="\uE70E")
            self.expanded = True
            self.scroll_frame.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
            # when expanding, always grow to the configured max height (or a sensible minimum)
            target_h = self.max_height
            # keep a lower bound in case header is tall
            target_h = max(target_h, self.min_height + 200)
            self.animate_height(self.winfo_height(), target_h)

    def animate_height(self, current_h, target_h, on_complete=None):
        # cancel any previous animation job so we don't stack
        ...
        # schedule next step and keep id
        self.anim_job = self.after(ANIMATION_SPEED_MS, lambda: self.animate_height(new_h, target_h, on_complete))

    def _update_header_only(self, tasks):
        """Csak a fejléc számait frissíti, a feladatlista érintetlen marad."""
        for w in self.title_container.winfo_children(): w.destroy()
        ...
        for t in tasks:
            if t['status'] == 'FOLYAMATBAN':
                ...
                if date_obj and date_obj < today:
                    expired += 1
        if CLEAN_STYLE:
            ctk.CTkLabel(self.title_container, text=f"{active} feladat",
                         font=FONT_TEXT_NORMAL, text_color="#EEE").pack(side="left", padx=5)
        else:
            ctk.CTkLabel(self.title_container, text=f"{active} Feladat",
                         font=FONT_TEXT_BOLD, text_color="white").pack(side="left")
            if expired > 0:
                ctk.CTkLabel(self.title_container, text=" | ",
                             font=FONT_TEXT_BOLD, text_color="gray").pack(side="left")
                ctk.CTkLabel(self.title_container, text=f"{expired} Lejárt",
                             font=FONT_TEXT_BOLD, text_color="#FF7777").pack(side="left")
    ...
    def start_refresh(self, skip_intro=False):
        ...
        threading.Thread(target=self.fetch_and_update, args=(skip_intro,), daemon=True).start()
    ...
    def _tasks_equal(self, a, b):
        if len(a) != len(b):
            return False
        for x, y in zip(a, b):
            if (x.get('id') != y.get('id') or x.get('status') != y.get('status')
                    or x.get('date') != y.get('date') or x.get('priority') != y.get('priority')
    ...
    def update_ui(self, tasks, skip_intro=False):
        ...
        # small optimization: if tasks identical to last fetch, do nothing
        ...
        if not isinstance(tasks, list):
            ...
            if isinstance(tasks, dict) and "error" in tasks:
                ctk.CTkLabel(self.title_container, text=f"⚠️ {tasks['error']}", font=FONT_TEXT_BOLD, text_color="orange").pack(side="left")
            return
    ...
    def show_intro_animation(self, tasks):
        for w in self.title_container.winfo_children(): w.destroy()
        intro_lbl = ctk.CTkLabel(self.title_container, text="Planner Widget ", font=FONT_TEXT_BOLD, text_color="#2b2b2b")
        intro_lbl.pack(side="left")
        check_lbl = ctk.CTkLabel(self.title_container, text="\uE73E", font=("Segoe MDL2 Assets", 13, "bold"), text_color="#2b2b2b")
        check_lbl.pack(side="left", padx=(0, 5))
        def fade(step):
            if step > 10:
                self.after(200, lambda: self.render_finished(tasks))  # reduced from 500ms
                return
            val = int(43 + (step * 21))
            color = f"#{val:02x}{val:02x}{val:02x}"
            intro_lbl.configure(text_color=color)
            if step > 5: check_lbl.configure(text_color="#00A2ED")
            self.after(15, lambda: fade(step + 1))  # reduced from 30ms
        fade(0)
    ...
    def render_task_status(self, tasks):
        ...
        if CLEAN_STYLE:
            ...
            ctk.CTkLabel(self.title_container, text=f"{active} Feladat", font=FONT_TEXT_BOLD, text_color="white").pack(side="left")
            if expired > 0:
                ctk.CTkLabel(self.title_container, text=" | ", font=FONT_TEXT_BOLD, text_color="gray").pack(side="left")
                ctk.CTkLabel(self.title_container, text=f"{expired} Lejárt", font=FONT_TEXT_BOLD, text_color="#FF7777").pack(side="left")
        
        # efficient update: only remove widgets that no longer exist
        ...
        for w in self.task_widgets.values(): w.grid_forget()
        if self.separator: self.separator.grid_forget()
        ...
        for task in sorted_tasks:
            ...
            if not sep_placed and task['status'] == "KESZ":
                if not self.separator: self.separator = ctk.CTkFrame(self.tasks_container, height=2, fg_color="#444444")
                self.separator.grid(row=_row_idx, column=0, sticky="ew", padx=10, pady=10)
                _row_idx += 1
                sep_placed = True
            self.task_widgets[tid].grid(in_=self.tasks_container, row=_row_idx,
                                        column=0, sticky="ew", pady=4, padx=2)
            _row_idx += 1

    def update_widget_content(self, frame, task):
        # Virtual DOM: csak akkor építjük újra a kártyát, ha valami tényleg változott
        if (getattr(frame, "current_status", None) == task.get("status") and
                getattr(frame, "current_title",  None) == task.get("title") and
                getattr(frame, "current_date",   None) == task.get("date") and
                getattr(frame, "current_priority", None) == task.get("priority")):
            return
    ...
    def build_task_card(self, fr, task):
        ...
        if CLEAN_STYLE:
            ...
            bg, border, title_col = "#1A1A1A", "#2b2b2b", "#E0E0E0"
            if is_active:
                title_col, bg, border = "white", "#2b2b2b", "#404040"
                date = task.get('date')
                if date and date != "Nincs határidő":
                    try:
                        diff = (datetime.strptime(date, "%Y-%m-%d").date() - datetime.now().date()).days
                        if diff < 0:
        ...
        fr.configure(fg_color=bg, border_color=border, border_width=0 if CLEAN_STYLE else 1)
        ...
        if CLEAN_STYLE:
            # only show date and a simple checkbox style
            if is_active:
                ctk.CTkLabel(bot, text=task.get('date',''), text_color="#AAA", font=FONT_TEXT_SMALL).pack(side="left")
                ctk.CTkButton(
                    bot,
                    text="✔",
                    width=20,
                    ...
                    hover_color="#333",
                    command=lambda tid=task['id'], ttitle=task['title']: self.mark_as_done(tid, ttitle),
                ).pack(side="right")
            else:
                ctk.CTkLabel(bot, text="✔", text_color="#555", font=FONT_TEXT_SMALL).pack(side="left")
        else:
            if is_active:
                ctk.CTkButton(bot, text="\uE73E", width=28, height=28, font=("Segoe MDL2 Assets", 12), fg_color="#333", hover_color="#00AA00", command=lambda tid=task['id'], ttitle=task['title']: self.mark_as_done(tid, ttitle)).pack(side="left", padx=(0, 5))
                ctk.CTkLabel(bot, text=f"📅 {task.get('date','-')}", text_color="#AAAAAA", font=FONT_TEXT_SMALL).pack(side="left", padx=5)
                p_map = {'high': ("Fontos", "#FF5555"), 'low': ("Alacsony", "#55AAFF"), 'medium': ("Közepes", "#AAAAAA")}
                ...
                ctk.CTkLabel(pc, text="●", text_color=p_col, font=("Arial", 10), width=12).pack(side="left")
                ctk.CTkLabel(pc, text=p_txt, text_color=p_col, font=("Segoe UI", 10, "bold")).pack(side="left", padx=(2, 0))
                ...
                chk = ctk.CTkFrame(bot, width=18, height=18, corner_radius=4, fg_color="transparent", border_width=1, border_color="#555")
